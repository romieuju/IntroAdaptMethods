#############################################ROC BY WINDOW######################################################################################
#Script used to obtain graphs representing the ROC curves for a test set with adjacente windows and auroc by adjacent window class
...
#adjacente window class :
vec_wind_genomatnn <- c(100000, 100000, 200000, 200000, 300000, 300000, 400000, 400000, 500000, 500000)
vec_wind_other     <- c(50000, 100000, 150000, 200000, 250000, 300000, 350000, 400000, 450000, 500000)
auroc_all<-data.frame("method"=c("gnn_1","gnn_2","mal_noenoZnS","mal_noQnoenoZnS","Q_1_100_q95","vol_rel"))
for (wind in 1:length(vec_wind_genomatnn)){
  ...
  predictiontable[predictiontable$method=="gnn_1",]$start = predictiontable[predictiontable$method=="gnn_1",]$start-1
  predictiontable[predictiontable$method=="gnn_2",]$start = predictiontable[predictiontable$method=="gnn_2",]$start-1
  predictiontable$dist_to_mut<-abs(predictiontable$start-500000)
  predictiontable_IA <- predictiontable[predictiontable$classifier==1,]
  predictiontable_window_other<-predictiontable[predictiontable$dist_to_mut==vec_wind_other[wind],]
  predictiontable_window_gnn<-predictiontable[(predictiontable$method=="gnn_1"| predictiontable$method=="gnn_2") & predictiontable$dist_to_mut==vec_wind_genomatnn[wind],]
  predictiontable<-rbind(predictiontable_IA,predictiontable_window_other,predictiontable_window_gnn)
  ...
  #pdf(file.path(eval_dir,paste("roc_leg_volcano_norm_max_all_IA_VS_ADJ",project,".pdf",sep="")),width = 900, height = 600)
  ...
  ploprocwindow <- ggplot(graphtablebis[graphtablebis$graph=="roc" & graphtable$x!=0 & graphtable$y!=0 & is.finite(graphtable$thresholds),],aes(x=x,y=y,group=method,shape=method))+
  ...
  ggsave(paste("ROC_AI_VS_",toString(vec_wind_other[wind]),"_wind_",project,".pdf",sep=""), ploprocwindow, width = 11.69, height = 8.27, units = "in", dpi = 300, device = "pdf")

  auroc_table<-summarytable[,c("method", "auroc")]
  names(auroc_table)[2] <- toString(vec_wind_other[wind])
  auroc_all <- merge(auroc_all,auroc_table,by="method")
}
#wite AUROC file by adjacent window class :
write.table(auroc_all, file =paste("auroc_AI_VS_ADJ_by_window_",project,".csv",sep=""), col.names = TRUE, row.names = FALSE)